...
def get_pretrained_controlnet_model(
    name: str, controlnet_types: List[str], depth_factor=0.5
):
    ...
    model_config["model"]["diffusion"]["config"]["controlnet_depth_factor"] = (
        depth_factor
    )
        ...
        if controlnet_type in ["audio", "envelope", "chroma"]:
            controlnet_conditioner_config = {
                ...
                "config": {
                    ...
                    "output_dim": model_config["model"]["pretransform"]["config"][
                        "latent_dim"
                    ],
...
if __name__ == "__main__":
    ...
    from main.data.dataset_musdb import collate_fn_conditional, create_musdb_dataset

    device = "cuda" if torch.cuda.is_available() else "cpu"

    model, model_config = get_pretrained_controlnet_model(
        "stabilityai/stable-audio-open-1.0", controlnet_types=["audio"]
    )
    model = model.cuda()
    ...
    dataset = create_musdb_dataset(
        "../../data/musdb18hq/train.tar", sample_rate=44100, chunk_dur=47.57
    )
