...
from main.chord_conditioner import ChromaChordConditioner, RotatedChromaChordConditioner
...
def test_rotated_chroma_conditioner_rotates_chroma_by_root():
    conditioner = RotatedChromaChordConditioner(
        output_dim=16, embed_dim=8, conv_channels=4
    )
...
def test_rotated_chroma_conditioner_sets_no_chord_flag():
    conditioner = RotatedChromaChordConditioner(
        output_dim=16, embed_dim=8, conv_channels=4
    )
    chord_data = torch.tensor(
...
def test_rotated_chroma_conditioner_forward_matches_target_size():
    conditioner = RotatedChromaChordConditioner(
        output_dim=32, embed_dim=16, conv_channels=8
    )
    chord_data = torch.tensor(
        [
            [0, _quality_index("maj"), 0],
...
def test_chroma_conditioner_does_not_rotate_chroma():
    ...
    # ChromaChordConditioner does NOT rotate chroma based on root
    expected_chroma[0, list(maj_intervals)] = 1
    expected_chroma[1, list(maj_intervals)] = 1
    expected_chroma[2, list(min_intervals)] = 1
