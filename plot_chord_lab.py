...
class ChordProgressionAnalyzer:
    ...
    def load_lab_file(self, file_path: str) -> pd.DataFrame:
        ...
        try:
            ...
            # コード解析
            chord_series = data["chord"].fillna("N").astype(str)
            parsed = [parse_chord_label(chord) for chord in chord_series]
...
class ChordVisualization:
    ...
    def plot_timeline_only(self, figsize=(16, 4), save_path: Optional[str] = None):
        ...
        ax.set_title(
            "Chord Progression Timeline", fontsize=18, fontweight="bold", pad=20
        )
        ax.set_yticks([])
    ...
    def plot_timeline(self, figsize=(16, 10), save_path: Optional[str] = None):
        ...
        ax1.set_title(
            "Chord Progression Timeline", fontsize=16, fontweight="bold", pad=20
        )
        ax1.set_yticks([])
        ...
        colors = [
            self.get_chord_color_fallback(quality) for quality in quality_counts.index
        ]
        bars = ax3.bar(
        ...
        if not valid_roots.empty:
            ...
            # 円グラフで表示
            colormap = plt.cm.get_cmap("Set3", len(root_counts))
            color_values = colormap(np.linspace(0, 1, len(root_counts)))
    ...
    def plot_circle_of_fifths(self, figsize=(10, 10), save_path: Optional[str] = None):
        ...
        ax.set_title(
            "Root Distribution (Circle of Fifths)",
            fontsize=14,
            fontweight="bold",
            pad=20,
        )
    ...
    def create_interactive_plot(self, save_path: Optional[str] = None):
        ...
        # レイアウトの設定
        fig.update_layout(
            title="Interactive Chord Progression Analysis", height=800, showlegend=False
        )
...
def main():
    ...
    try:
        ...
        analyzer.load_lab_file(args.lab_file)
